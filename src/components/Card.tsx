...
interface cardProps extends cardObjects {
  className: string;
}

const Card: React.FC<cardProps> = ({ title, subtitle, rating, backgroundColors, imageSrc, className }) => {
  const { top, bottom } = backgroundColors;
  return (
    <div
      className={
        "card flex flex-col justify-center p-5 rounded-[10%] relative text-white overflow-hidden h-fit w-fit z-10 hover:-translate-y-10 transition-all duration-300" +
        className
      }
      style={{
    ...
    >
      <div className="inset-0 absolute z-0">
        <img src="/public/assets/bg.png" alt="background" className="w-full h-full object-cover opacity-10" />
      </div>
      ...
      <img src={imageSrc} alt={title} className="w-52 h-52 object-contain mt-5 drop-shadow-[0_2px_2px_rgba(0,0,0,0.3)] mx-5 mb-7" />
    </div>
  );
};
